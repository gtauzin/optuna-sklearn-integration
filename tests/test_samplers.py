...
import optuna_sklearn_integration
from optuna_sklearn_integration.botorch import logei_candidates_func
from optuna_sklearn_integration.botorch import qei_candidates_func


parametrize_sampler = pytest.mark.parametrize(
    "sampler_class",
    [
        lambda: optuna_sklearn_integration.BoTorchSampler(
            n_startup_trials=0,
        ...
        lambda: optuna_sklearn_integration.PyCmaSampler(n_startup_trials=0),
    ],
)
...
parametrize_multi_objective_sampler = pytest.mark.parametrize(
    ...
    [
        lambda: optuna_sklearn_integration.BoTorchSampler(n_startup_trials=0),
    ],
)


sampler_class_with_seed: dict[str, tuple[Callable[[int], BaseSampler], bool]] = {
    "BoTorchSampler": (lambda seed: optuna_sklearn_integration.BoTorchSampler(seed=seed), True),
    "PyCmaSampler": (lambda seed: optuna_sklearn_integration.PyCmaSampler(seed=seed), True),
}
